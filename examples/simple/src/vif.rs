...
use crate::vr_type::utils::{add_vhost_ip, create_vhost};
...
pub fn init_ifaces() -> Result<Vec<Message>, MessageHandleError> {
    ...
    add_xc_iface(state.alloc()).unwrap();
    add_veth_iface(VM1_IFNAME, state.alloc()).unwrap();
    add_veth_iface(VM2_IFNAME, state.alloc()).unwrap();
...
pub fn add_veth_iface(name: &str, idx: i32) -> Result<Vec<Message>, MessageHandleError> {
    ...
    vifr.flags = VIF_FLAG_L2_ENABLED | VIF_FLAG_MAC_LEARN | VIF_FLAG_UNKNOWN_UC_FLOOD;
    vifr.mtu = 1514;
...
pub fn find_ipaddr(name: &str) -> Result<Ipv4Addr, ()> {
    if let Some(iface) = interfaces()
        .iter()
        .find(|iface: &&NetworkInterface| iface.name == name.to_string())
    {
